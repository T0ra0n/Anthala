...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $project_scope = htmlspecialchars(trim($_POST['project_scope'] ?? ''));
    $project_size = htmlspecialchars(trim($_POST['project_size'] ?? ''));
    ...
    if (!empty($_FILES['quoteFiles'])) {

        foreach ($_FILES['quoteFiles']['tmp_name'] as $key => $tmp_name) {
            $file_error = $_FILES['quoteFiles']['error'][$key];
            ...
            $file_size = $_FILES['quoteFiles']['size'][$key];
            $original_name = $_FILES['quoteFiles']['name'][$key];
    ...
    $subject = "Quote Request for: $service";
    ...
    $email_body .= "Project Type: " . ($project_type ?: 'Not specified') . "\n";
    $email_body .= "Business Area: " . ($project_scope ?: 'Not specified') . "\n";
    $email_body .= "Project Size: " . ($project_size ?: 'Not specified') . "\n";
    $email_body .= "Estimated Timeline: " . ($timeline ?: 'Not specified') . "\n";
    $email_body .= "Estimated Budget: " . ($budget ?: 'Not specified') . "\n\n";
    ...
    if ($mail_sent) {
        if (!empty($cleanup_errors)) {
            error_log("Quote request sent but failed to delete some files: " . implode(', ', $cleanup_errors));
        }
        echo "success";
    } else {
        http_response_code(500);
        echo "An error occurred while sending the quote request. Please try again later.";
